const AI_TERMS = /\b(ai|ml|llm|genai|machine learning|deep learning|pytorch|tensorflow|nlp|agent|embedding|vector|rag)\b/i;
const SAP_TERMS = /\b(sap|s\/4hana|s4hana|abap|fico|bw\/4|btp|successfactors)\b/i;
export function applyDomainFilters(jobs, filters) {
    if (!filters)
        return jobs;
    let out = jobs;
    if (filters.ai) {
...
export function applyQueryFilters(jobs, opts) {
    let out = jobs;
    if (opts.remote === true) {
