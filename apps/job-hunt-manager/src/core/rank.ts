...
export function rankJobs(jobs: UnifiedJob[], keywordQuery: string, trackKeywords: string[]): UnifiedJob[] {
  ...
  const scored = jobs.map((j) => {
    ...
    for (const t of hTokens) {
      if (qTokens.has(t)) hits += 2;
    }
    for (const q of qTokens) {
      if (q.length >= 4 && hay.includes(q)) hits += 1;
    }
