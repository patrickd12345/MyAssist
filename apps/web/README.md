...
- **Root Directory:** `apps/web` (pnpm monorepo; includes workspace package `job-hunt-manager`). The repo **root** `package.json` defines **`vercel-build`** → `pnpm --filter web run build` so if Vercel runs the build from the monorepo root, `next build` still targets `apps/web` (which has `app/`).
