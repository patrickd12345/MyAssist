...
const productionSmokeSpecRequested = process.argv.some((arg) => arg.includes("production-smoke.spec"));
const productionSmokeBaseUrl = process.env.PLAYWRIGHT_BASE_URL ?? process.env.NEXT_PUBLIC_SITE_URL ?? "";
const productionSmokeEnabled = productionSmokeSpecRequested || Boolean(productionSmokeBaseUrl);
const baseURL = productionSmokeEnabled ? productionSmokeBaseUrl : e2eOrigin;
...
export default defineConfig({
  ...
  webServer: productionSmokeEnabled
    ? undefined
    : {
