...
import {
  buildTodayInsights,
  isInterviewLikeCalendarEvent,
} from "@/lib/services/todayIntelligenceService";
import { CommunicationDraftToolbar } from "./CommunicationDraftToolbar";
import {
  AssistantConsole,
  type CommunicationDraftInjectPayload,
...
function firstMessageIdFromInsightActions(
...
function integrationFeedbackPanelClass(
  outcome: UnifiedActionFeedback["outcome"],
): string {
  if (outcome === "failed") {
    ...
    <ul className="mt-3 space-y-3">
      {items.map((insight) => {
        ...
        return (
          <li
            key={insight.id}
            className="list-card rounded-[18px] border px-3 py-3 text-sm leading-6"
          >
            ...
            {insight.description &&
            insight.description !== insight.explanation ? (
              <p className="theme-muted mt-1 text-[11px] leading-5 opacity-90">
                {insight.description}
              </p>
            ) : null}
            {actions.length > 0 ? (
...
function buildNextAction(data: MyAssistDailyContext): {
  ...
  if (overdue) {
    return {
      title: overdue,
      detail:
        "This is already late. Closing it buys back trust and mental bandwidth immediately.",
      cue: "Overdue task",
    };
  }
  ...
  if (dueToday) {
    return {
      title: dueToday,
      detail:
        "This lands today. Put it on calendar before the rest of the day expands around it.",
      cue: "Due today",
    };
  }
  ...
  return {
    ...
    detail:
      "No obvious fire surfaced in this pull. Use the runway for work that actually matters.",
    cue: "Clear runway",
  };
}
...
function buildJobHuntHrefFromOpportunityLinkage(
  link: JobHuntCalendarOpportunityLink,
): string {
...
function CompactMetricCard({
  ...
  const inner = (
    <span className="flex flex-wrap items-baseline gap-x-1.5">
      ...
      <span className="tabular-nums text-xl font-semibold tracking-tight theme-ink">
        {value}
      </span>
    </span>
  );
  ...
  return (
    <div className="grid min-w-0 gap-2 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <CompactMetricCard
        label="Job"
        value={m.job}
        ariaLabel="Job-related signals count from briefing"
      />
    </div>
  );
}
...
function OpinionList({
  ...
  return (
    <section className="glass-panel min-w-[280px] self-start rounded-[28px] p-5">
      <h2 className="section-title mb-4 text-xs font-semibold">{title}</h2>
      {isLoading ? (
...
function SkeletonBlock({ className }: { className: string }) {
  return (
    <span
      className={`block rounded-lg bg-white/10 animate-pulse ${className}`}
      aria-hidden
    />
  );
}
  ...
  return (
    ...
    >
      {Array.from({ length: 4 }).map((_, i) => (
        <div
          key={`metric-skel-${i}`}
          className="metric-chip rounded-[18px] border border-white/10 px-3 py-3 sm:px-4"
        >
...
export function Dashboard({
  ...
  const [contextSource, setContextSource] =
    useState<DailyContextSource>(initialSource);
  ...
  const [pendingCrossActionKeys, setPendingCrossActionKeys] = useState<
    string[]
  >([]);
  const [ignoredJobHuntMessageIds, setIgnoredJobHuntMessageIds] = useState<
    string[]
  >([]);
  const [taskActionError, setTaskActionError] = useState<string | null>(null);
  const [integrationFeedback, setIntegrationFeedback] =
    useState<UnifiedActionFeedback | null>(null);
  ...
  const [situationBrief, setSituationBrief] = useState<SituationBrief | null>(
    null,
  );
  const [briefLoading, setBriefLoading] = useState(false);
  const [briefError, setBriefError] = useState<string | null>(null);
  const [briefFeedbackState, setBriefFeedbackState] = useState<
    "idle" | "sending" | "saved"
  >("idle");
  ...
  const [gmailReadTogglePendingKey, setGmailReadTogglePendingKey] = useState<
    string | null
  >(null);
  const [energyLevel, setEnergyLevel] = useState<"high" | "normal" | "low">(
    "normal",
  );
  const [activeTab, setActiveTab] = useState<DashboardTab>("overview");
  const router = useRouter();
  const [pendingInsightActionKeys, setPendingInsightActionKeys] = useState<
    string[]
  >([]);
  ...
  const lastHeadlineKeyRef = useRef<string | null>(
    initialData?.generated_at ?? null,
  );
  ...
  const [communicationDraftLang, setCommunicationDraftLang] =
    useState<DraftLanguage>("en");
  ...
  const [lastSilentRefreshError, setLastSilentRefreshError] = useState<
    string | null
  >(null);
    ...
    const loadResolvedItems = async () => {
      try {
        ...
        const body = (await response.json()) as {
          resolved_items?: ResolvedMemoryItem[];
        };
        if (!response.ok) {
        ...
        if (!cancelled) {
          setResolvedItems(
            Array.isArray(body.resolved_items) ? body.resolved_items : [],
          );
        }
      } catch {
        if (!cancelled) {
  ...
  useEffect(() => {
    if (!data) {
      setHeadline(
        loading
          ? "Analyzing your day..."
          : "MyAssist is getting your day ready.",
      );
        ...
        if (!cancelled) {
          ...
          setBriefError(
            cause instanceof Error
              ? cause.message
              : "Could not load the situation brief.",
          );
        }
      } finally {
        if (!cancelled) {
  ...
  const refresh = useCallback(
    async (options?: { mode?: "blocking" | "silent" }) => {
      const mode =
  ...
  const loadCachedSnapshot = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        "/api/daily-context?source=cache",
  ...
  const refreshProviderSlice = useCallback(async (provider: ProviderSlice) => {
    try {
      const res = await fetch(
      ...
      setData((previous) => {
        ...
        if (
          provider === "gmail" &&
          "gmail_signals" in body &&
          Array.isArray(body.gmail_signals)
        ) {
          ...
          if (
            body.daily_intelligence &&
            typeof body.daily_intelligence === "object"
          ) {
            merged.daily_intelligence = body.daily_intelligence;
          }
          if (
            body.unified_daily_briefing &&
            typeof body.unified_daily_briefing === "object"
          ) {
            merged.unified_daily_briefing = body.unified_daily_briefing;
          }
          if (
            body.good_morning_message &&
            typeof body.good_morning_message === "object"
          ) {
        ...
        if (
          ...
          const merged: MyAssistDailyContext = {
            ...previous,
            calendar_today: body.calendar_today,
          };
          if (
            body.calendar_intelligence &&
            typeof body.calendar_intelligence === "object"
          ) {
        ...
        if (
          ...
          return {
            ...
            ...(body.todoist_intelligence
              ? { todoist_intelligence: body.todoist_intelligence }
              : {}),
            ...(body.unified_daily_briefing
              ? { unified_daily_briefing: body.unified_daily_briefing }
              : {}),
            ...(body.good_morning_message
              ? { good_morning_message: body.good_morning_message }
              : {}),
          };
        }
        return previous;
  ...
  const recoverCreatedTarget = useCallback(
    async (targetId: string, kind: "calendar" | "todoist") => {
      ...
      try {
        ...
        await refreshProviderSlice(
          kind === "calendar" ? "google_calendar" : "todoist",
        );
        ...
        setIntegrationFeedback({
          ...
          message:
            cause instanceof Error ? cause.message : "Could not remove item.",
          dismissible: true,
        });
      } finally {
  ...
  const applyActionsResponseBody = useCallback(
    async (body: ActionApiResponseBody) => {
      ...
      const feedback = buildFeedbackFromActionResponse(
      ...
      if (feedback) {
        ...
        if (body.opportunityLinkage) {
          merged = {
            ...feedback,
            href: buildJobHuntHrefFromOpportunityLinkage(
              body.opportunityLinkage,
            ),
          };
        }
      ...
      await Promise.all(
        unique.map((provider) => refreshProviderSlice(provider)),
      );
      void loadActionHistory();
    },
  ...
  const runCrossSystemAction = useCallback(
    async (
      ...
      try {
        ...
        setPendingCrossActionKeys((current) =>
  ...
  const runInsightAction = useCallback(
    async (insightId: string, action: SuggestedAction) => {
      ...
      setTaskActionError(null);
      setIntegrationFeedback(null);
      setPendingInsightActionKeys((current) => [
        ...new Set([...current, pendingKey]),
      ]);
      try {
        ...
        if (result.outcome === "navigate_tab") {
          if (
            ...
            const signal =
              mid && ctx ? findGmailSignalByMessageId(ctx, mid) : undefined;
  ...
  const assignEmailToJob = useCallback(
    async (signal: GmailSignal, jobId: string) => {
      ...
      try {
        ...
        setAssignError(
          cause instanceof Error
            ? cause.message
            : "Could not assign this email to a saved job.",
        );
      } finally {
  ...
  const completeTask = useCallback(
    async (taskId: string) => {
      ...
      setData({
        ...data,
        todoist_overdue: data.todoist_overdue.filter(
          (task) => String(task.id ?? "") !== taskId,
        ),
        todoist_due_today: data.todoist_due_today.filter(
          (task) => String(task.id ?? "") !== taskId,
        ),
        todoist_upcoming_high_priority:
          data.todoist_upcoming_high_priority.filter(
            (task) => String(task.id ?? "") !== taskId,
          ),
      });
  ...
  const scheduleTask = useCallback(
    async (taskId: string, dueString: string, intent?: string) => {
      ...
      const taskContent =
        taskRow && typeof taskRow.content === "string"
          ? taskRow.content.trim()
          : "";
      ...
      try {
        const response = await fetch(
          `/api/todoist/tasks/${encodeURIComponent(taskId)}/schedule`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
  ...
  const visibleEmailSignals = useMemo(
    () =>
      (data?.gmail_signals ?? []).filter(
        (signal) =>
          !resolvedKeySet.has(
            `email:${normalizeMemoryKey(signal.subject ?? "")}`,
          ),
      ),
    [data, resolvedKeySet],
  );
  ...
  const nextAction = useMemo(
    () => (displayData ? buildNextAction(displayData) : null),
    [displayData],
  );
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!cancelled) {
          setProactiveError(
            cause instanceof Error ? cause.message : "Proactive load failed",
          );
          setProactiveIntel(null);
        }
      } finally {
  ...
  const needsFirstRefresh = Boolean(
    bootstrapped && !data && !error && !loading,
  );
    ...
    async (
      ...
      try {
        ...
        setResolvedItems((current) => [
          {
            ...
            feedback:
              source === "email"
                ? (emailResolution ?? "useful_action")
                : undefined,
          },
          ...current.filter(
            (item) =>
              !(item.source === source && item.normalized === normalized),
          ),
        ]);
        if (source === "email" && (emailSignal?.id || emailSignal?.threadId)) {
          const markReadResponse = await fetch("/api/gmail/mark-read", {
            ...
            body: JSON.stringify({
              ...
              ...(emailSignal?.threadId
                ? { threadId: emailSignal.threadId }
                : {}),
            }),
          });
          if (!markReadResponse.ok) {
            const markBody = (await markReadResponse.json()) as {
              error?: string;
            };
            setTaskActionError(
              markBody.error ??
        ...
        setTaskActionError(
          cause instanceof Error
            ? cause.message
            : "Could not save handled item.",
        );
      } finally {
        setPendingResolvedTexts((current) =>
    ...
    async (g: GmailSignal) => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          setTaskActionError(
            body.error ??
              "Gmail mark-as-unread did not complete. Try again after reconnecting Gmail.",
          );
        } else {
          await refreshProviderSlice("gmail");
        }
        ...
        setTaskActionError(
          cause instanceof Error
            ? cause.message
            : "Could not mark as unread in Gmail.",
        );
      } finally {
  ...
  return (
    ...
    >
      <section className="glass-panel-strong mb-6 min-w-0 overflow-hidden rounded-[32px] px-5 py-5 sm:px-7 sm:py-6">
            ...
            <h1 className="theme-ink mt-4 text-balance text-3xl font-semibold tracking-[-0.04em] sm:text-4xl">
              {data ? (
                ...
                <>
                  ...
                  <span className="theme-ink mt-2 block animate-pulse">
                    {headline}
                  </span>
                </>
              ) : needsFirstRefresh ? (
                <>
                  ...
                  <span className="mt-2 block">
                    No saved snapshot yet. Refresh pulls live Gmail, Calendar,
                    and Todoist and saves a snapshot for faster next visits.
                  </span>
                </>
              ) : (
          ...
          <div className="flex flex-col gap-3 xl:min-w-[440px] xl:items-end">
            <div className="flex gap-3 relative">
              ...
              <div className="theme-selector relative">
                ...
                >
                  Theme:{" "}
                  {THEMES.find((option) => option.key === theme)?.label ??
                    "Light"}
                </button>
                {themeMenuOpen ? (
        ...
        <div className="mt-6 min-w-0 space-y-3 sm:space-y-4">
          {!showSkeleton && displayData?.good_morning_message ? (
            <div className="rounded-2xl border border-white/15 bg-gradient-to-br from-white/[0.09] to-white/[0.02] px-5 py-4 shadow-sm shadow-black/15 sm:px-6 sm:py-5">
              ...
              <p className="theme-muted mt-2 text-sm leading-relaxed">
                {displayData.good_morning_message.message}
              </p>
            </div>
          ) : null}
          ...
          {!showSkeleton && displayData ? (
            <UnifiedDailyBriefingPanel
              briefing={displayData.unified_daily_briefing}
            />
          ) : null}
          ...
          {!showSkeleton && displayData ? (
            <>
              ...
              <CalendarIntelligencePanel
                intel={displayData.calendar_intelligence}
              />
            </>
          ) : null}
        </div>
      ...
      <section
        className="glass-panel mb-6 rounded-[24px] p-4"
        aria-label="Integration status"
      >
        <div className="flex flex-wrap items-center gap-2 text-xs">
          <span className="theme-muted mr-2 font-semibold uppercase tracking-[0.14em]">
            Integrations
          </span>
          {(
            ["gmail", "todoist", "google_calendar"] as IntegrationProvider[]
          ).map((provider) => {
            const row = integrationStatuses.find(
          ...
          <div className="ml-auto flex flex-wrap gap-2">
            {integrationStatuses.find((x) => x.provider === "gmail")?.status !==
              "connected" && (
              <button
                type="button"
                onClick={() => {
            ...
            {integrationStatuses.find((x) => x.provider === "todoist")
              ?.status !== "connected" && (
              <button
                type="button"
                onClick={() => {
            ...
            {integrationStatuses.find((x) => x.provider === "google_calendar")
              ?.status !== "connected" && (
              <button
                type="button"
                onClick={() => {
      ...
      {contextSource === "demo" && data && !error && (
        ...
        >
          ...
          <p className="mt-1 text-xs leading-6 opacity-90">
            Curated demo context —{" "}
            <code className="rounded bg-white/5 px-1.5 py-0.5">
              MYASSIST_DEMO_MODE
            </code>{" "}
            is enabled. No live provider calls.
          </p>
        </div>
      )}
      ...
      {contextSource === "mock" && data && !error && (
        <div
          className="glass-panel mb-6 rounded-[24px] border-blue-500/30 bg-blue-500/10 p-5 text-blue-100"
          role="status"
        >
          <p className="font-semibold">Demo mode is active.</p>
          <p className="mt-1 text-xs leading-6 opacity-90">
            The interface is using demo data because{" "}
            <code className="rounded bg-white/5 px-1.5 py-0.5">
              MYASSIST_USE_MOCK_CONTEXT
            </code>{" "}
            is enabled on the server. Turn it off and connect Gmail, Calendar,
            and Todoist for live data.
          </p>
        </div>
      )}
      ...
      {contextSource === "cache" && data && !error && (
        ...
        >
          ...
          <p className="mt-1 text-sm leading-6 text-emerald-900/90">
            Snapshot last captured {formatWhen(data.generated_at)}. Use Refresh
            to pull live provider data again and replace this snapshot.
          </p>
        </div>
      )}
      ...
      {needsFirstRefresh && (
        <div
          className="glass-panel mb-6 rounded-[24px] border-white/10 p-5"
          role="status"
        >
          ...
          <p className="theme-muted mt-2 text-sm leading-6">
            ...
            . Later visits load that file first so Today opens immediately; only
            Refresh triggers a new run.
          </p>
          <button
            type="button"
            onClick={() => void refresh()}
      ...
      {error && (
        <div
          role="alert"
          className="glass-panel mb-6 rounded-[24px] border-red-500/30 p-5 text-sm text-red-300"
        >
          ...
          <p className="mt-3 text-xs leading-6">
            Check that Gmail, Google Calendar, and Todoist are connected, then
            try Refresh again.
          </p>
        </div>
      )}
      ...
      {integrationFeedback && !error && (
        ...
        >
          ...
          {integrationFeedback.createdTargets &&
          integrationFeedback.createdTargets.length > 0 ? (
          ...
          {integrationFeedback.reusedTargets &&
          integrationFeedback.reusedTargets.length > 0 ? (
            <div className="mt-3 flex flex-wrap gap-2">
          ...
          {actionHistoryRows.length === 0 ? (
            <p className="theme-muted mt-3 text-xs leading-6">
              {actionHistoryLoading
                ? "Loading action log…"
                : "No logged actions yet. Run an automation from insights or inbox."}
            </p>
          ) : (
                ...
                >
                  <div className="flex flex-wrap items-baseline justify-between gap-2">
                    ...
                    <span className="theme-muted shrink-0">
                      {formatActionHistoryTime(row.timestamp)}
                    </span>
                  </div>
                    ...
                    {row.outcome === "failed"
                      ...
                      : row.outcome === "deduped"
                        ...
                        : row.status === "success"
                          ...
                          : row.status}
                    {row.sourceIds[0]
                      ? ` · source ${row.sourceIds[0].slice(0, 24)}${row.sourceIds[0].length > 24 ? "…" : ""}`
                      : ""}
                  </p>
                      ...
                      {row.recoverableTargets.map((t) => {
                        ...
                        return (
                          <button
                            ...
                            disabled={busy}
                            onClick={() =>
                              void recoverCreatedTarget(t.id, t.kind)
                            }
            ...
            <div className="space-y-6">
                ...
                <div className="space-y-6">
                  ...
                  >
                    ...
                    {proactiveError && !proactiveIntel ? (
                      <p
                        className="theme-muted mt-2 text-xs leading-6"
                        role="status"
                      >
                    ...
                    {proactiveLoading && !proactiveIntel && !proactiveError ? (
                      <p className="theme-muted mt-3 text-sm animate-pulse">
                        Loading briefing…
                      </p>
                    ) : null}
                    {proactiveIntel ? (
                  ...
                  >
                    ...
                    <p className="theme-muted mt-2 text-xs leading-6">
                      Compared to the last time this dashboard synced proactive
                      intel (no background jobs).
                    </p>
                        ...
                        <ul className="mt-4 space-y-2 text-xs">
                          {proactiveIntel.changesSinceLastVisit.map((c) => (
                            ...
                            >
                              ...
                              <p className="theme-ink mt-1 font-medium">
                                {c.title}
                              </p>
                              {c.detail ? (
              ...
              {activeTab === "overview" && todayInsights ? (
                ...
                >
                  ...
                  <p className="theme-muted mt-2 text-sm leading-7">
                    From the current daily context: calendar, tasks, and Gmail
                    signals (no background jobs).
                  </p>
                  {dailySynthesis ? (
                    <div className="mt-4 rounded-[20px] border border-zinc-500/25 bg-zinc-500/[0.07] px-4 py-3 text-sm">
                      ...
                      <p className="theme-ink mt-2 font-medium leading-6">
                        {dailySynthesis.oneLineSummary}
                      </p>
                    ...
                    <div>
                      ...
                      <TodayInsightList
                        ...
                        renderAfterActions={(insight, actions) => {
                          ...
                          if (!mid) return null;
                          const sig = findGmailSignalByMessageId(
                            displayData,
                            mid,
                          );
                          return (
              ...
              jobHuntExpansion ? (
                ...
                >
                  ...
                  <p className="theme-muted mt-2 text-sm leading-7">
                    From job-hunt signals, Todoist wording, and normalized
                    identity — same live context as Today Insights.
                  </p>
                  <div className="mt-5 grid gap-5 lg:grid-cols-3">
                    <div>
                      <p className="theme-accent text-[11px] font-semibold uppercase tracking-[0.12em]">
                            ...
                            >
                              ...
                              {typeof f.daysSinceTouch === "number" ? (
                                <p className="theme-muted mt-1 text-[11px]">
                                  ~{f.daysSinceTouch} day
                                  {f.daysSinceTouch === 1 ? "" : "s"} since
                                  email date
                                </p>
                              ) : null}
                              {(() => {
                    ...
                    <div>
                      ...
                      {jobHuntExpansion.researchSuggestions.length === 0 ? (
                        <p className="theme-muted mt-3 text-sm">
                          Need company and role on a signal
                        </p>
                      ) : (
                        <ul className="mt-3 space-y-3">
                              ...
                              {(() => {
                                ...
                                return (
                                  <button
                                    ...
                                    onClick={() =>
                                      void runInsightAction(rowId, openJh)
                                    }
              ...
              displayData.job_hunt_email_matches.length > 0 ? (
                ...
                >
                  ...
                  <p className="theme-muted mt-2 text-sm leading-7">
                    Gmail signals matched to job-hunt-manager saved leads
                    (touchpoints logged locally).
                  </p>
                  <ul className="mt-4 space-y-3">
                    {displayData.job_hunt_email_matches.map(
                      (m: JobHuntEmailMatch, i: number) => (
                        <li
                          key={`${m.job_id}-${i}`}
                          className="list-card rounded-[22px] px-4 py-3 text-sm leading-6"
                        >
                          <div className="flex flex-wrap items-baseline justify-between gap-2">
                            <p className="font-semibold text-zinc-100">
                              {m.company}
                              <span className="theme-muted font-normal">
                                {" "}
                                · {m.title}
                              </span>
                            </p>
                            <span className="signal-pill rounded-full px-2 py-0.5 text-[11px] font-medium">
                              ...
                              {m.match_reason.replace(/_/g, " ")}
                            </span>
                          </div>
                          <p className="theme-muted mt-1 text-xs">
                            {typeof m.signal.from === "string"
                              ? m.signal.from
                              : String(m.signal.from ?? "")}{" "}
                            — {m.signal.subject}
                          </p>
                ...
                <section className="glass-panel rounded-[30px] p-6">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        disabled={
                          briefFeedbackState === "sending" || !situationBrief
                        }
                  ...
                  {briefLoading ? (
                    ...
                    <div className="mt-5 space-y-4 text-sm">
                      <div className="list-card rounded-[22px] px-4 py-4">
                        <p className="theme-accent text-[11px] uppercase tracking-[0.14em]">
                          Day shape
                        </p>
                        <p className="theme-ink mt-2 leading-6">
                          {situationBrief.pressure_summary}
                        </p>
                      </div>
                      <div className="grid gap-3 lg:grid-cols-2">
                        <div className="list-card rounded-[22px] px-4 py-4">
                          ...
                          <ul className="theme-ink mt-2 space-y-2">
                            {situationBrief.top_priorities.map((item, idx) => (
                              <li key={`priority-${idx}`} className="leading-6">
                                {item}
                              </li>
                            ))}
                          </ul>
                        ...
                        <div className="list-card rounded-[22px] px-4 py-4">
                          ...
                          <ul className="theme-ink mt-2 space-y-2">
                            {situationBrief.conflicts_and_risks.map(
                              (item, idx) => (
                                <li key={`risk-${idx}`} className="leading-6">
                                  {item}
                                </li>
                              ),
                            )}
                      ...
                      <div className="grid gap-3 lg:grid-cols-2">
                        <div className="list-card rounded-[22px] px-4 py-4">
                          ...
                          <ul className="theme-ink mt-2 space-y-2">
                            {situationBrief.defer_recommendations.map(
                              (item, idx) => (
                                <li key={`defer-${idx}`} className="leading-6">
                                  {item}
                                </li>
                              ),
                            )}
                        ...
                        <div className="list-card rounded-[22px] px-4 py-4">
                          ...
                          <ul className="theme-ink mt-2 space-y-2">
                            {situationBrief.next_actions.map((item, idx) => (
                              <li key={`action-${idx}`} className="leading-6">
                                {item}
                              </li>
                            ))}
                          </ul>
                      ...
                      <div className="list-card rounded-[22px] px-4 py-4">
                        ...
                        <p className="theme-muted mt-2 leading-6">
                          {situationBrief.confidence_and_limits}
                        </p>
                        {situationBrief.memory_insights.length > 0 ? (
                          <div className="mt-3">
                            ...
                            <ul className="theme-ink mt-2 space-y-2">
                              {situationBrief.memory_insights.map(
                                (item, idx) => (
                                  <li
                                    key={`memory-${idx}`}
                                    className="leading-6"
                      ...
                      {briefFeedbackState === "saved" ? (
                        <p className="theme-muted text-xs">
                          Feedback saved for future brief tuning.
                        </p>
                      ) : null}
                    </div>
                  ) : (
              ...
              {activeTab === "tasks" ? (
                <section
                  id="tasks"
                  className="glass-panel scroll-mt-6 rounded-[30px] p-6"
                >
                  <div className="flex flex-wrap items-center justify-between gap-4">
                    <div>
                      ...
                      <p className="theme-muted mt-2 text-sm leading-6">
                        Overdue, today, and highlights from the situation brief
                        — same live Todoist actions.
                      </p>
                    </div>
                    <div className="flex flex-wrap gap-2">
                  ...
                  <div className="mt-6 grid items-start gap-5 lg:grid-cols-2 2xl:grid-cols-3">
                    <div className="list-card rounded-[22px] px-4 py-4 lg:col-span-2 2xl:col-span-3">
                      <p className="theme-accent text-[11px] uppercase tracking-[0.14em]">
                        Todoist intelligence
                      </p>
                      <p className="theme-muted mt-2 text-sm leading-6">
                        {displayData.todoist_intelligence?.summary ??
                          "No Todoist intelligence yet."}
                      </p>
                    </div>
                    <TaskList
                      title="Overdue"
                      ...
                      onBlockCalendar={(taskId) =>
                        runCrossSystemAction("task_to_calendar_block", taskId)
                      }
                    ...
                    <OpinionList
                      ...
                      onResolve={async (item) =>
                        resolveMemoryItem(item, "priority")
                      }
              ...
              {activeTab === "inbox" ? (
                <section
                  id="important-emails"
                  className="glass-panel scroll-mt-6 rounded-[30px] p-5"
                >
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <p className="section-title text-xs font-semibold">
                      ...
                      <p className="theme-muted mt-1 max-w-xl text-xs leading-5">
                        Inbox (all categories), last 10 days — priority rows are
                        highlighted; everything else stays visible below.
                      </p>
                    </div>
                    <span className="signal-pill rounded-full px-3 py-1 text-[11px] font-semibold">
                      {displayData.gmail_signals.length}
