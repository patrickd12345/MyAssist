...
**Related deep-dives:** [auth-supabase-callback.md](./auth-supabase-callback.md) (callback flow, error codes, session cookies) · [pre-demo-smoke.md](./pre-demo-smoke.md) · [commercial-pilot-readiness.md](./commercial-pilot-readiness.md) · [infisical-and-secrets.md](./infisical-and-secrets.md) (Infisical paths, MCP, CLI, vault sync) · `apps/web/README.md` (Playwright, app env, Vercel)
...
- **Source of truth for team dev:** [Infisical](https://infisical.com) — **`/platform` + `/myassist`**, `dev` env (and prod as applicable). **End-to-end runbook:** [infisical-and-secrets.md](./infisical-and-secrets.md) (MCP, CLI, `sync-env-to-infisical-once.mjs`, rotation).
...
> **Context:** MyAssist Next.js app (`apps/web`), Supabase Auth, PKCE `/auth/callback`, `ensureAppUser` to `myassist.app_users`, pnpm-only, Infisical `/platform`+`/myassist` — [infisical-and-secrets.md](./infisical-and-secrets.md) for runbook.  
> **Local / demo with team secrets:** `infisical init` in `apps/web`, then **`pnpm dev:infisical`** or root **`pnpm dev:all`** — not plain `pnpm dev` alone if you expect Infisical-injected `AUTH_SECRET`, `NEXT_PUBLIC_SITE_URL`, and Supabase keys.  
...
> **If bug:** state symptom, host (local vs Vercel), whether you used **dev:infisical**, and whether `NEXT_PUBLIC_SITE_URL` / Supabase Site URL are set. Read [docs/TROUBLESHOOTING.md](./TROUBLESHOOTING.md) and [auth-supabase-callback.md](./auth-supabase-callback.md).
